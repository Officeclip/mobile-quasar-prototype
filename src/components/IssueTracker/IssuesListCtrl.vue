...
const $q = useQuasar();
const props = defineProps(['parent', 'binderId', 'binderName']);
...
const viewDetails = async (id: string, binderName: string, appName: any) => {
  await router.push({
    ...
    <q-list v-for="issue in issuesList" :key="issue.id">
      ...
      > -->
        <q-item>
          ...
          <q-item-section v-if="issue.status" side>
