  ...
  <div>
    <div class="q-pa-md">
      <div class="q-gutter-y-md column">
        ...
        <q-select name="newtaskDate" label="Date" v-model="taskDate" :options="dateOptions" map-options emit-label />
        <pre>{{ sampleModel }}</pre>
        ...
        <q-select label="Billable" v-model="props.timesheet.isBillable" :options="billableOptions" map-options
          emit-value />
        <q-input label="Duration" v-model="props.timesheet.timeDuration" placeholder="enter here..." lazy-rules
        ...
        <q-input label="Comments" v-model="props.timesheet.description" placeholder="enter here..." lazy-rules
          :rules="[(val) => (val && val.length > 0) || 'Please type something']">
        </q-input>
      </div>
    </div>
